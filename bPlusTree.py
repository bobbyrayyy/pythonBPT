...
class BPlusTree(object):
    ...
    # Added to show only top layer
    def showTop(self, remaining_levels, node=None, file=None, _prefix="", _last=True):
        ...
        if type(node) is Node:
            ...
            for i, child in enumerate(node.values):
                _last = (i == len(node.values) - 1)
                self.showTop(remaining_levels-1, child, file, _prefix, _last)
